  ...
  adminCode: [
    {
      validator: (rule, value, callback) => {
        if (loginForm.value.role === 'super_admin' && !value) {
          callback(new Error('请输入管理员令牌'))
        } else {
          callback()
        }
      },
    ...
    {
      validator: (rule, value, callback) => {
        if (loginForm.value.role === 'super_admin' && value && value.length !== 8) {
          callback(new Error('管理员令牌必须为8位'))
        } else {
          callback()
        }
      },
